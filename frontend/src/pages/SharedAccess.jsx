    ...
    useEffect(() => {
        const fetchFile = async () => {
            try {
                ...
                if (!resJson.success) {
                    throw new Error(resJson.message || "Cryptographic link invalid or expired.");
                }
    ...
    useEffect(() => {
        ...
        const checkExpiry = () => {
            if (Date.now() > new Date(fileData.expiresAt).getTime()) {
                setFileData(null);
                setError("Cryptographic token expired. Internal mechanisms have severed access securely.");
            }
        };
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', animation: 'fadeUp 0.6s ease', marginTop: '2rem' }}>
            ...
            {loading ? (
                <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>Decrypting temporal link payload...</p>
                </div>
            ) : error ? (
                <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px solid rgba(244,63,94,0.3)', boxShadow: '0 0 40px rgba(244,63,94,0.1)' }}>
                    ...
                    <h2 className="text-gradient" style={{ color: 'var(--error)' }}>Payload Rejected</h2>
                ...
                <div className="glass-panel" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-muted)', marginTop: '0.5rem', fontSize: '0.9rem' }}>
                            Mass: {(fileData.fileSize / 1024 / 1024).toFixed(2)} MB &bull; Encoding: {fileData.fileType.split('/')[1] || fileData.fileType}
                        </p>
                        ...
                        <div style={{ margin: '2rem 0', padding: '2rem', background: 'rgba(0,0,0,0.4)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <p style={{ marginBottom: '1.5rem', fontSize: '0.95rem', color: fileData.permission === 'VIEW' ? 'var(--warning)' : 'var(--text-pure)' }}>
                                Clearance Level Granted: <strong style={{ padding: '0.2rem 0.5rem', borderRadius: '4px', background: 'rgba(255,255,255,0.1)' }}>{fileData.permission}</strong>
                            </p>
                                    ...
                                    {fileData.fileType.startsWith('image/') ? (
                                        ...
                                        <>
                                            ...
                                            <div
                                                ...
                                                title="Extracted viewing disabled per strict security policy"
                                            />
                                        </>
                                    )}
                                </div>
                        ...
                        <div className="badge badge-blue" style={{ marginTop: '1rem', padding: '0.5rem 1rem' }}>
                            Connection severed at: {new Date(fileData.expiresAt).toLocaleString()}
                        </div>
                    </div>
                </div>
            )}
